 ...
 * @author DHRUV CHOPRA
 */
public class WebSocketProtobufEncoder extends ProtocolEncoderAdapter {
    ...
    public void encode(IoSession session, Object message, ProtocolEncoderOutput out) throws Exception {
        ...
        if (isHandshakeResponse) {
            ...
            resultBuffer = WebSocketProtobufEncoder.buildWSResponseBuffer(response);
        } else if (isDataFramePacket) {
            IoBuffer buffer = toIoBuffer(message);

            WebSocketCodecPacket packet = WebSocketCodecPacket.buildPacket(buffer);
            resultBuffer = isRemoteWebSocket ? WebSocketProtobufEncoder.buildWSDataFrameBuffer(packet.getPacket()) : packet
                    .getPacket();
        } else {
    ...
    private IoBuffer toIoBuffer(Object message) throws IOException {
